      ...
      <div className="space-y-20 pb-20">
        ...
        <section className="relative overflow-hidden rounded-3xl bg-stone-900 px-8 py-20 md:px-16 md:py-28">
          ...
          <div className="relative z-10 max-w-2xl">
            <div className="hero-badge inline-flex items-center gap-2 px-3 py-1.5 bg-amber-600/20 border border-amber-500/30 rounded-full mb-6">
            ...
            <h1
              className="hero-title text-4xl md:text-6xl font-bold text-amber-50 leading-tight tracking-tight"
            ...
            <p className="hero-sub mt-5 text-base md:text-lg text-stone-400 leading-relaxed max-w-lg">
          ...
          <div className="hero-coconut absolute right-8 top-1/2 -translate-y-1/2 hidden lg:flex flex-col items-center gap-2 select-none pointer-events-none">
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {stats.map(({ value, label }, i) => (
            <div
              key={label}
              data-animate
              className="fade-up bg-white rounded-2xl border border-stone-100 shadow-sm px-6 py-5 text-center
                hover:shadow-md hover:-translate-y-0.5 transition-all duration-200"
              style={{ transitionDelay: `${i * 80}ms` }}  // ← transitionDelay not animationDelay
            >
              <p className="text-3xl font-bold text-stone-800" style={{ fontFamily: "'Georgia', serif" }}>
                {value}
              </p>
        ...
        {/* ── CATEGORIES ── */}
        <section>
          <div data-animate className="fade-up flex items-center gap-4 mb-8">
            <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {categories.map(({ emoji, label, desc }, i) => (
              <Link
                key={label}
                href={`/products?category=${label.toLowerCase()}`}
                data-animate
                className="fade-up group bg-white rounded-2xl border border-stone-100 p-5
                  hover:border-amber-200 hover:shadow-lg hover:shadow-amber-900/8
                  hover:-translate-y-1 transition-all duration-200"
                style={{ transitionDelay: `${i * 60}ms` }}  // ← transitionDelay
              >
        ...
        {/* ── FEATURED PRODUCTS ── */}
        <section>
          <div data-animate className="fade-up flex items-center justify-between mb-8">
            <div>
        ...
        <section
          data-animate
          className="fade-up rounded-3xl bg-amber-50 border border-amber-100 px-8 py-12 md:px-14 flex flex-col md:flex-row items-center gap-8"
        >
          <div className="text-5xl shrink-0">🌿</div>
          <div className="flex-1">
            <p className="text-xs font-bold uppercase tracking-[3px] text-amber-700 mb-2">Why Coconut Coir?</p>
            <h3 className="text-xl font-bold text-stone-800 mb-2" style={{ fontFamily: "'Georgia', serif" }}>
