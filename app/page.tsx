...
export default function HomePage() {
  return (
    <div className="space-y-20 pb-20">
      ...
      {/* ── WELCOME BANNER (shows only when logged in) ── */}
      <WelcomeBanner />
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {stats.map(({ value, label }) => (
          <div key={label}
            ...
            <p className="text-3xl font-bold text-stone-800" style={{ fontFamily: "'Georgia', serif" }}>{value}</p>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {categories.map(({ emoji, label, desc }) => (
            <Link key={label} href={`/products?category=${label.toLowerCase()}`}
              ...
              <p className="font-semibold text-stone-800 text-sm group-hover:text-amber-700 transition-colors">{label}</p>
