...
export default function CheckoutPage() {
  ...
  const validate = () => {
    ...
    else if (!/^(09|\+639)\d{9}$/.test(phone.replace(/\s/g, ''))) e.phone = 'Enter a valid PH number'
    if (deliveryMethod === 'delivery' && !address.trim()) e.address = 'Delivery address is required'
    setErrors(e)
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-stone-800" style={{ fontFamily: "'Georgia', serif" }}>Checkout</h1>
        <p className="text-sm text-stone-400 mt-0.5">Review your order and complete your purchase</p>
      </div>
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-stone-200 p-5 space-y-4">
            <h2 className="font-bold text-stone-800 flex items-center gap-2">
              <span className="w-6 h-6 rounded-full bg-stone-800 text-white text-xs flex items-center justify-center font-bold">1</span>
              Contact Information
            </h2>
          ...
          <div className="bg-white rounded-2xl border border-stone-200 p-5 space-y-4">
            <h2 className="font-bold text-stone-800 flex items-center gap-2">
              <span className="w-6 h-6 rounded-full bg-stone-800 text-white text-xs flex items-center justify-center font-bold">2</span>
              Delivery Method
            </h2>
            ...
            {deliveryMethod === 'delivery' && (
              <div>
                <label className="text-xs font-semibold text-stone-600 mb-1 block">Delivery Address</label>
                <textarea
                  value={address}
            ...
            {deliveryMethod === 'pickup' && (
              <div className="flex items-start gap-3 p-3 rounded-xl bg-amber-50 border border-amber-100">
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-700 mt-0.5">123 Coir Street, Brgy. Sample, Manila, Philippines</p>
                  <p className="text-xs text-amber-600 mt-0.5">Mon–Sat, 8:00 AM – 5:00 PM</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-stone-200 p-5 space-y-4">
            <h2 className="font-bold text-stone-800 flex items-center gap-2">
              <span className="w-6 h-6 rounded-full bg-stone-800 text-white text-xs flex items-center justify-center font-bold">3</span>
              Payment Method
            </h2>
            ...
            {paymentMethod === 'gcash' && (
              <div className="p-4 rounded-xl bg-blue-50 border border-blue-100 space-y-1">
                <p className="text-xs font-bold text-blue-800">GCash Payment Details</p>
                <p className="text-xs text-blue-700">Send payment to: <span className="font-semibold">09XX XXX XXXX</span></p>
                <p className="text-xs text-blue-700">Account name: <span className="font-semibold">Juan Dela Cruz</span></p>
                <p className="text-xs text-blue-600 mt-2">After placing your order, please send payment and message us your order ID as reference.</p>
              </div>
            )}

            {paymentMethod === 'bank' && (
              <div className="p-4 rounded-xl bg-green-50 border border-green-100 space-y-1">
                <p className="text-xs font-bold text-green-800">Bank Transfer Details</p>
                <p className="text-xs text-green-700">Bank: <span className="font-semibold">BDO</span></p>
                <p className="text-xs text-green-700">Account number: <span className="font-semibold">1234 5678 9012</span></p>
                <p className="text-xs text-green-700">Account name: <span className="font-semibold">Cocoir-Mart</span></p>
                <p className="text-xs text-green-600 mt-2">After placing your order, please transfer the exact amount and use your order ID as reference.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-2xl border border-stone-200 p-5 space-y-4 sticky top-24">
            ...
            <div className="border-t border-stone-100 pt-3 space-y-2 text-sm">
              ...
              <div className="flex justify-between text-stone-500">
                <span>Shipping</span>
                <span className="text-green-600 font-medium">{deliveryMethod === 'pickup' ? 'Free' : 'Free'}</span>
              </div>
